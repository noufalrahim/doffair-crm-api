...
"""
from vendor.schemas.prescription_v2 import UnifiedPrescriptionResponse
from vendor.services.prescription_data_service import list_prescriptions_data
...
async def upload_prescription(
    ...
    try:
        ...
        logger.info(
            f"✅ Prescription created: {prescription.id} for booking {booking_id}")
...
async def get_prescription_by_id(
    ...
    try:
        ...
        logger.error(
            f"Error fetching prescription {prescription_id}: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def get_unified_prescriptions_by_booking(
    engine: AIOEngine,
    vendor_id: str,
    booking_id: str
) -> dict:
    """
    Get all uploaded prescriptions and structured prescription data for a booking.
    ...
    Returns:
        Dict matching UnifiedPrescriptionResponse schema
    """
